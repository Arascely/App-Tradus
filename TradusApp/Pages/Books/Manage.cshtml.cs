...
public class ManageModel : PageModel
{
    ...
    // Nuevo libro
    [BindProperty] public BookCreateDto NewBook { get; set; } = new();
    ...
    public async Task<IActionResult> OnPostCreateBookAsync()
    {
        ...
        if (string.IsNullOrWhiteSpace(NewBook.Titulo)) ModelState.AddModelError("NewBook.Titulo", "Título requerido");
        if (string.IsNullOrWhiteSpace(NewBook.Autor)) ModelState.AddModelError("NewBook.Autor", "Autor requerido");
        if (string.IsNullOrWhiteSpace(NewBook.Idioma)) ModelState.AddModelError("NewBook.Idioma", "Idioma requerido");
        if (!ModelState.IsValid)
        {
            await OnGetAsync();
            return Page();
        ...
        await _uow.Repository<Libro>().AddAsync(libro);
    ...
    public async Task<IActionResult> OnPostCreateChapterAsync()
    {
        ...
        var libro = await _uow.Repository<Libro>().Query().Include(l => l.Capitulos).FirstOrDefaultAsync(l => l.Id == Book.NewChapter.LibroId);
        if (libro == null) return RedirectToPage();

        var nextNumero = libro.Capitulos.Any() ? libro.Capitulos.Max(c => c.Numero) + 1 : 1;
        if (Book.NewChapter.Numero <= 0) Book.NewChapter.Numero = nextNumero;
